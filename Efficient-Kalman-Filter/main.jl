...
function nm_on_chunk(ψ::NTuple{6,Any}, ins::KalmanData{Float64}, idxr::UnitRange{Int})
    ...
    a0_new, Σx_new, Σw_new, Σv_new, θF_new, θg_new =
      EKF.NM(
        ...
        a0, Σx, Σw, Σv, θF, θg;
        tol=1e5, maxiter=5, verbose=true,
...
function rolling_optimize(ins::KalmanData{Float64}, outs::KalmanData{Float64}, ψ0::NTuple{6,Any})
    ...
    chunk_sz = max(1, floor(Int, 0.001 * total_t)) #3% works on CJ's Mac
    ...
    for (ci, idxr) in enumerate(ranges)
        ...
        mse_cand, mae_cand, _ = compute_ins_mse(ψ_cand, ins, "Newton Nr: $ci")
...
zPredRKFALL = outs.zPredAll 
outputData.write_results(zPredNMAll, zPredRKFALL);
